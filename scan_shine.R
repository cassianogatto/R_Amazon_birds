            ...
            tabItem("scan",
                    ...
                    fluidRow(
                        ...
                        box(tags$h5("Check Cs data table columns (sp1, sp2, Cs)"),
                            ...
                            tags$h5("The results are in the datasets (see preview below)"),
...
server <- function(input,output,session){
    ...
    output$names_scan <- renderUI({
        ...
        selectInput(inputId = "dataset", label = "Choose a dataset to download", choices = SCANlist() |> names() )
    })
    ...
    datasetInput <- reactive({
        switch(input$dataset,
               ...
               "graph_nodes" = SCANlist()[['graph']] |> activate(nodes) |> as_tibble(),
               "graph_edges" = SCANlist()[['graph']] |> ativate(edges) |> as_tibble()
        )
    })
