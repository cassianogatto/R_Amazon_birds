...
shinyServer(function(input,output,session){
    ...
    output$Cs_tail <- renderTable(    if(nrow(Cs()) > 0){    Cs() |> tail()    } else { return()    }    )
    ...
    # graph <- eventReactive(input$make_graph, {
    # 
    ...
    # # output$graph_nodes <- renderTable(  graph() |> activate(nodes) |> as_tibble()  )
    # # output$graph_edges <- renderTable( graph() |> activate(edges) |> as_tibble())
    # 
